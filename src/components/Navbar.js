...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        (mobileAvatarMenuRef.current && !mobileAvatarMenuRef.current.contains(event.target))
      ) {
  ...
  const UserAvatar = ({ isMobile = false }) => (
    <div className="relative" ref={isMobile ? mobileAvatarMenuRef : avatarMenuRef}>
      <button
      ...
      {isAvatarMenuOpen && (
        <div
          className={`${
            isMobile 
              ? 'fixed left-1/2 transform -translate-x-1/2 top-auto' 
              : 'absolute right-0 lg:right-0 md:right-0 sm:right-[-4rem] xs:right-[-6rem]'
          } ${
            isMobile ? 'mt-2' : 'mt-3'
          } w-64 max-w-[calc(100vw-1rem)] bg-gradient-to-br from-slate-800 via-slate-900 to-gray-900 rounded-2xl shadow-2xl border border-slate-700/50 backdrop-blur-xl z-[60] transition-all duration-500 ease-out transform ${
            isAvatarMenuOpen ? 'scale-100 opacity-100 translate-y-0' : 'scale-95 opacity-0 -translate-y-2'
          } overflow-hidden`}
        >
          ...
          {/* Header section */}
          <div className="relative px-4 sm:px-6 py-4 border-b border-slate-700/50 bg-gradient-to-r from-slate-800/50 to-slate-700/50" onClick={() => setIsAvatarMenuOpen(false)}>
            <div className="flex items-center space-x-3">
              <div className="w-8 h-8 rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 flex items-center justify-center text-white font-bold text-sm shadow-lg flex-shrink-0">
                {getInitials(username)}
              </div>
          ...
          {isAvatarMenuOpen &&
            <div className="relative py-2 bg-gradient-to-b from-slate-900/50 to-slate-800/50">
              <a
                ...
                className="group flex items-center px-4 sm:px-6 py-3 text-sm font-medium text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-all duration-300 focus:outline-none focus:bg-red-500/10 relative overflow-hidden"
              >
                ...
                {/* Logout icon */}
                <svg className="w-4 h-4 mr-3 group-hover:animate-bounce flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                </svg>

                <span className="relative z-10 group-hover:translate-x-1 transition-transform duration-300 truncate">
                  Logout
                </span>

                {/* Arrow indicator */}
                <svg className="w-3 h-3 ml-auto opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all duration-300 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
